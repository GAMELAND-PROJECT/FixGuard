...
class EncryptedWarpIdentityStore(private val context: Context) {
    fun load(): WarpIdentity? {
        ...
        return try {
            ...
            decode(JSONObject(String(cipher.doFinal(Base64.decode(envelope.getString("data"), Base64.NO_WRAP)), Charsets.UTF_8)))
    ...
    fun save(identity: WarpIdentity) {
        ...
        val encrypted = cipher.doFinal(encode(identity).toString().toByteArray(Charsets.UTF_8))
